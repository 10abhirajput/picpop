...
const model = "card_payment";
const modelBaseName = "transaction";
const modelTitle = "Transactions";
const modelImageFolder = "card_payment";
    ...
    try {
      ...
      const listing = await models[model].findAll({
        ...
        include: [
          ...
          },
          {
            ...
            on: {
              col1: sequelize.where(
                sequelize.col("card_payment.locationOwnerId"),
                "=",
          ...
          },
          {
            ...
            on: {
              col1: sequelize.where(
                sequelize.col("card_payment.serviceProviderId"),
                "=",
      ...
      const data = await Promise.all(
        listing.map(async (row, index) => {
          ...
          orderStatusSelect += `<select class="locationStatus" model="${model}" model_title="${modelTitle}" model_id="${
            row.id
          }" field="statuslocation"  style="background: ${
            statuslocation[row.statuslocation].backgroundCssColor
          }; color: #fff; font-weight: 600;" >`;
          for (let status in statuslocation) {
            orderStatusSelect += `<option value="${status}" ${
              status == row.statuslocation ? "selected" : ""
            } style="background: ${
              statuslocation[status].backgroundCssColor
            };  color: #fff; font-weight: 600;" >${
              statuslocation[status].value
            }</option>`;
          }
orderStatusSelect += `</select>`;
          ...
          let businesStatusSelect = ``;
          businesStatusSelect += `<select class="locationStatus" model="${model}" model_title="${modelTitle}" model_id="${
            row.id
          }" field="statusbusines"  style="background: ${
            statuslocation[row.statusbusines].backgroundCssColor
          }; color: #fff; font-weight: 600;" >`;
          for (let status in statusbusines) {
            businesStatusSelect += `<option value="${status}" ${
              status == row.statusbusines ? "selected" : ""
            } style="background: ${
              statuslocation[status].backgroundCssColor
            };  color: #fff; font-weight: 600;" >${
              statuslocation[status].value
          ...
          return Object.values({
            ...
            locationComission: row.locationComission,
            serviceProviderPrice: row.serviceProviderPrice,
            businesComission: row.businesComission,
            statusbusines: businesStatusSelect,
            statuslocation: orderStatusSelect,
            // action,
          });
        })
      );
      ...
      const headerColumns = Object.values({
        ...
        locationComission: "LocationComission",
        serviceProviderPrice: "serviceProviderPrice",
        businesComission: "ProviderComission",
        statusbusines: "Busines Status",
        statuslocation: "Provider Status",
