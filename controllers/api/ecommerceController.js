...
module.exports = {
    ecommerceHomeListing: async (req, res) => {
        try {
            const required = {
            ...
            return helper.success(res, `${modelName} sub category listing based on categories fetched successfully.`, responseData);
        } catch (err) {
            return helper.error(res, err);
        }
    },
    ecommerceSubCategoryListing: async (req, res) => {
        try {
            const required = {
                securitykey: req.headers.securitykey,
            ...
            const categories = await models['category'].findAll({
                where: {
                    parentId: requestData.categoryId,
                },
                attributes: [
                    'id',
                    'name',
            ...
            const responseData = [
                ...
                {
                    title: 'EXPLORE BY BRANDS',
                    data: [],
                }
            ]

            return helper.success(res, `${modelName} home listing fetched successfully.`, responseData);
        } catch (err) {
            return helper.error(res, err);
        }
    },
